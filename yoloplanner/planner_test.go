...
func TestFormatQuestionResponses(t *testing.T) {
	tests := []struct {
		name      string
		questions []interface{}
		responses []string
		...
		{
			name: "single question response",
			questions: []interface{}{
				map[string]interface{}{"question": "Pick one?"},
			},
			responses: []string{"Option A"},
		...
		{
			name: "more responses than questions",
			questions: []interface{}{
				map[string]interface{}{"question": "Q1?"},
			},
			responses: []string{"A1", "A2", "A3"},
			expected:  "User responses:\n- Q: Q1?\n  A: A1\n",
		},
		{
			name: "more questions than responses",
			questions: []interface{}{
				map[string]interface{}{"question": "Q1?"},
				map[string]interface{}{"question": "Q2?"},
		...
		{
			name: "invalid question map is skipped but response index still advances",
			questions: []interface{}{
				"not a map",
				map[string]interface{}{"question": "Valid?"},
			},
			responses: []string{"SkippedAnswer", "ValidAnswer"},
			expected:  "User responses:\n- Q: Valid?\n  A: ValidAnswer\n",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := formatQuestionResponses(tt.questions, tt.responses)
			if result != tt.expected {
				t.Errorf("expected %q, got %q", tt.expected, result)
			}
		})
