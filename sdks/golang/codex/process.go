...
// processManager manages the Codex CLI process (app-server mode).
...
// BuildCLIArgs builds the CLI arguments for the app-server.
func (pm *processManager) BuildCLIArgs() ([]string, error) {
	args := []string{
		"app-server",
	}

	// Use -c config overrides for model and other settings
	if pm.config.Model != "" {
		args = append(args, "-c", "model=\""+pm.config.Model+"\"")
	}

	if pm.config.ReasoningEffort != "" {
		args = append(args, "-c", "reasoning_effort="+pm.config.ReasoningEffort)
	}

	if pm.config.ReasoningSummary != "" {
		args = append(args, "-c", "reasoning_summary=\""+pm.config.ReasoningSummary+"\"")
	}
